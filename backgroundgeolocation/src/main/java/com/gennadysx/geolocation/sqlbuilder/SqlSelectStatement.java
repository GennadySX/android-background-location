...
package com.gennadysx.geolocation.sqlbuilder;

import com.gennadysx.geolocation.utils.SQLTextUtils;
...
public class SqlSelectStatement implements SqlStatement {
    ...
    */
    public String statement() {
        ...
        if (_column.size() > 0) {
            b.append(SQLTextUtils.join(", ", _column));
        } else {
            b.append(_all);
        }
        ...
        if (_table.size() > 0) {
            b.append(" FROM ");
            b.append(SQLTextUtils.join(", ", _table));
        }
        ...
        for (Object join : _join) {
            ...
            if (joinCondition.length > 0) {
                ...
                if (joinCondition.length > 0) {
                    b.append(" USING (").append(SQLTextUtils.join(", ", joinCondition)).append(")");
                }
            }
        }
        ...
        if (_groupBy.size() > 0) {
            b.append(" GROUP BY ").append(SQLTextUtils.join(", ", _groupBy));
        }
        ...
        if (_orderBy.size() > 0) {
            b.append(" ORDER BY ").append(SQLTextUtils.join(", ", _orderBy));
        }
